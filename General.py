...
# +) 코딩스타일: 이해하기 쉽고 가독성이 좋은 코드가 좋은 스타일을 가진 코드.
...
# divmod() 함수(몫과 나머지 한번에 계산)
print(divmod(50, 8))  # (6, 2)  # 50을 8로 나눈 몫 6과 나머지 2가 한번에 계산됨.


# 1-1> 소수형 연산: 어느 하나라도 소수형이면 결과도 소수형.
# 덧셈
print(4.0 + 6.0)  # 10.0
...
# 3) 집합 자료형 관련 함수들
# 값 1개 추가(add): '.add()'  이미 만들어진 set 자료형에 값 추가.
# 1개의 값만 추가 할 경우
s1 = set([1, 2, 3])
s1.add(4)
print(s1)  # {1, 2, 3, 4}

# 값 여러 개 추가(update): '.update()'
s1 = set([1, 2, 3])
s1.update([4, 5, 6])
print(s1)  # {1, 2, 3, 4, 5, 6}

# 특정 값 제거(remove): '.remove()'
s1 = set([1, 2, 3])
s1.remove(2)
print(s1)  # {1, 3}
...
# 2> 파라미터(Parameter,매개변수): 함수를 정의할때 쓰는 변수(함수에 넘겨주는 값)
...
# 2-2) 키워드 파라미터 kwargs: 키워드 파라미터 사용시 매개변수 앞 별 두 개(**)를 붙임.
...
# 4> 'return 문': 함수에 어떤정보를 주면 특정 값을 반환(돌려줌) + 함수를 즉시 종료시킴.
# 예시1)
def get_square(c):
    return c * c
...
# 9> 상수(Constant): 변하지 않고, 항상 일정한 값을 가지는 수. 변수이름의 모든 글자 '대문자'!
...
# [제어문]
# 1> 'while 반복문': 무언가를 반복하기 위해 사용. 조건이 '참'인 동안 실행하는 반복문. 코드에서 루프를 빠져나올때는 break사용.

# while <조건부분>:  # 그 명령들을 실행하기 위한 조건. # 불린 값으로 계산되는 식 = 'True'일 동안 실행.
#    <수행부분>  # 반복적으로 수행하고 싶은 명령들. # 출력할 수 있고 변수 등을 수정할 수도 있음.

# while <다운로드 안 받은 이미지가 있다>: # = 사이트에 있는 모든 이미지를 다 다운로드 받으면, 이 반복문을 끝내도 좋다.
#    <다음 이미지를 보고, 다운로드 받는다>
# > 만약 사이트에 이미지를 다 다운로드 받으면, 조건부분을 충족하지 않기 때문에 반복문은 자동으로 종료.
...
# 2> 'if...else~'문: if(만약), else(그렇지 않으면). 상황별 다른 동작을 위해 사용.
# while문처럼 반복하지않고, 딱 한번만 조건문을 확인. 조건을 판단해 해당 조건에 맞는 상황 수행.

# if 조건부분:  # 불린값으로 계산되는 식. # 조건 충족시 명령실행.
#     실행부분  # 조건 충족시, 실행할 명령.
# else:       # 만약 위 조건을 충족하지 않으면 아래 수행부분 수행.(else문만 단독 사용불가)
#     수행부분
...
#    if 이미지가 png 파일:
#        이미지 다운로드
#    else:   # else는 필요에따라 선택적으로 사용(사용하지 않아도 됨)
#        print("png 파일이 아닙니다!")

# 예시)
temperature = 8
if temperature <= 10:
    print("자켓을 입는다.")  # 결과: 자켓을 입는다.

temperature = 16
if temperature <= 10:
    print("자켓을 입는다.")
else:
    print("자켓을 벗는다.")  # 결과: 자켓을 벗는다.


# 3> 재미있는 조건문: x in s, x not in s
#      in     /	   not in
...
# "[1, 2, 3]리스트에 1이 있는가?" 조건문. 1은 [1, 2, 3]안에 있으므로 참 > True를 돌려줌.
# "[1, 2, 3]리스트에 1이 없는가?" 조건문. 1은 [1, 2, 3]안에 있으므로 거짓 > False를 돌려줌.

# 예시2) 튜플과 문자열에 적용시,
print('a' in ('a', 'b', 'c'))  # True
print('j' not in 'python')  # True

# 예시3) "만약 주머니에 돈이 있으면 택시타고, 없으면 걸어가라"
pocket = ['paper', 'cellphone', 'money']
if 'money' in pocket:
    print("택시 타고가라")
else:
    print("걸어가라")  # 택시 타고가라
# ['paper', 'cellphone', 'money'] 리스트에 'money'가 있으므로 'money' in pocket은 참. 따라서 if문 다음문장이 수행.


# 4> [조건문에서 아무일도 하지않게 설정하려면? 'pass']
# 조건문의 참/거짓에 따라 실행할 행동을 정의하면서, 아무일도 하지않도록 설정하고 싶을 때.
# "주머니에 돈이 있으면 가만히 있고, 주머니에 돈이 없으면 카드를 꺼내라."
# 이럴때 사용하는 것이 바로 pass. 위 예를 pass를 적용해 구현해 보자.
...
if 'money' in pocket:
    ...
    print("카드를 꺼내라")  # ...
# pocket리스트에 money 문자열이 있기때문에, if문 다음문장인 'pass가 수행'되고 아무 결괏값도 보여주지 않음.

# +)이렇게 수행할 문장이 한 줄일 때, 더 간략하게 코드를 작성하는 방법:
pocket = ['paper', 'money', 'cellphone']
if 'money' in pocket: pass
else: print("카드를 꺼내라")  # ...
# if문 다음 수행할 문장을 콜론(:) 바로뒤에 적는다. else문 역시 마찬가지.


# 5> 조건부 표현식(conditional expression): 조건문이 참인 경우 if 조건문, else 조건문이 거짓인 경우.
...
if score >= 60:
    ...
    message = "failure"
# 위 코드는 score 60 이상이면 message 문자열 "success"를, 아닐경우 "failure"를 대입하는 코드.

# 파이썬의 조건부 표현식(conditional expression)적용:
message = "success" if score >= 60 else "failure"


# 6> 'elif 문': 수행부분이 여러개지만, 단 하나의 수행부분만 실행될 수 있음. 다중조건 판단을 가능하게 함.
# else..if..가 반복되는걸 줄여 'elif'로 표현. elif는 갯수에 제한없이 사용할 수 있다.
...
if score >= 90:
    ...
    print("F")  # 결과: F
# 수행부분이 5개나 있지만, '단 하나의 수행부분만이 실행'될 수 있음 > elif 문의 핵심.


# elif는 이전 조건문이 거짓일 때 수행됨. if, elif, else를 모두 사용할 때 기본구조는 다음과 같다.
...
# 7> 'break문': 만약 while문의 조건부분과 상관없이 '반복문에서 나오고 싶으면' break문 사용.(강제로 빠져나가기)
# 예시1)
i = 100
while True:
    if i % 23 == 0:  # i가 23의 배수면 반복문을 끝냄(빠져나옴).
        break
    i = i + 1
print(i)  # 결과: 115

# 추가질문) 이 코드에서 while의 조건부분에 해당하는 True는 무엇이 참인 것을 의미하나요?
# 답: 무엇이 참이라기 보다 그냥 참을 의미하는 'Boolean값 그 자체'를 의미. while문 다음에 나오는 'while표현식 부분이 True로 평가되는 것들'을 사용.
# False, None, 0, '', (), [], {}
# 이런 것들이 False로 평가되는데, 이외의 값들, 예를 들어 while 1과 같은 것들은 '1'이 'True'로 평가되어 같은의미로 사용가능.
...
# 콘솔에 나타나는 입력란 '돈을 넣어 주세요: '에 금액을 입력하면 명령에 따라 작동함.


# 8> 'continue 문': 현재 진행중인 수행부분을 중단하고, '바로 조건부분을 확인'하고 싶으면 continue문 사용.
# 예시1)
i = 0
while i < 15:
    i = i + 1

    if i % 2 == 1:  # i가 홀수면 print(i) 하지않고, 다시 조건부분으로 돌아감.
        continue
print(i)  # 결과: 2, 4, 6, 8, 10, 12, 14

# 예시2) 1부터 10까지의 숫자 중 홀수만 출력.
a = 0
while a < 10:
    a = a + 1
    if a % 2 == 0:  # a가 짝수면 print(a) 하지않고, 다시 조건부분으로 돌아감.
        continue
print(a)  # 결과: 1, 3, 5, 7, 9

# 해설: a가 10보다 작은동안 a는 1씩 증가한다. if a % 2 == 0(a를 2로 나누었을 때 나머지가 0인 경우)이 참인 경우는 a가 짝수일 때. 즉 a가 짝수면 continue 문장을 수행.
# continue문은 while문의 맨 처음(조건문: a<10)으로 돌아가게 하는 명령어. 따라서 위 예에서 a가 짝수면 print(a)는 수행되지 않음.


# 9> 무한루프(endless loop): 무한히 반복한다. 우리가 사용하는 일반 프로그램 중 무한루프 개념을 사용않는 프로그램이 거의없을 정도로 자주 사용한다.
# 파이썬에서 무한루프는 while문으로 구현할 수 있다. 다음은 무한 루프의 기본 형태이다.

# while True:
#     수행할 문장1
#     수행할 문장2
#     ...
# while문의 조건문이 True이므로 항상 참 > 따라서 while문 안에 있는 문장들은 무한히 수행됨.
...
# Ctrl+C를 눌러야 while문을 빠져나갈 수 있습니다.
# ....
# 위 문장은 영원히 출력된다. 하지만 이 예처럼 아무 의미없이 무한루프를 돌리는 경우는 거의없다. [Ctrl+C]를 눌러 빠져나가자.
...
#     수행할 문장2
#     ...
# 리스트, 튜플, 문자열의 첫 요소부터 마지막 요소까지 차례로 변수에 대입해 "수행할 문장1", "수행할 문장2" 등이 수행됨.
...
for i in test_list:
    print(i)
# one two three (콘솔 출력시 아래로 순서대로 나옴)
# ['one', 'two', 'three'] 리스트의 첫 요소인 'one'이 먼저 i변수에 대입된 후 print(i) 문장 수행.
# 다음 두번째 요소 'two'가 i변수에 대입후 print(i) 문장을 수행, 리스트의 마지막 요소까지 반복.
...
for (first, last) in a:
    print(first + last)
# 3 7 11 (콘솔 출력시 아래로 순서대로 나옴)
# 위는 a리스트의 요솟값이 튜플이라 각 요소가 자동으로 (first, last)변수에 대입됨.
# ※ 이 예는 앞서 살펴본 튜플을 사용한 변수값 대입방법과 매우 비슷.
(first, last) = (1, 2)

# 10-3. for문의 응용
# "총 5명이 시험을 봤다. 시험점수가 60점 이상은 합격, 그렇지않으면 불합격. 결과를 보여주시오."
# 우선 5명의 시험점수를 리스트로 표현 > marks = [90, 25, 67, 45, 80]
# 1번 학생은 90점이고 5번 학생은 80점. 점수를 검사해 합격, 불합격 여부를 통보하는 프로그램을 만들자.

marks = [90, 25, 67, 45, 80]

number = 0
for mark in marks:
    number = number + 1  # 학생에게 번호 부여.
    if mark >= 60:
...
# 4번 학생은 불합격입니다.
# 5번 학생은 합격입니다.
# 각 학생에게 번호부여 위해 number변수 사용. 점수리스트 marks에서 차례로 점수를 꺼내 mark라는 변수에 대입, for문 안의 문장수행.


# 10-4. 'for문' 과 'continue'
# for문 안의 문장을 수행하는 도중 continue문을 만나면, for문의 처음으로 돌아간다.
# 앞의 예제를 가져와 60점 이상이면 축하메시지, 나머지에게는 아무 메시지도 전하지 않는 프로그램.
...
# 3번 학생 축하합니다. 합격입니다.
# 5번 학생 축하합니다. 합격입니다.
# 60점 이하면 mark < 60이 참이므로 continue문 수행. 따라서 print문을 수행않고 for문의 처음으로 돌아감.


# 10-5. for문과 함께 자주 사용하는 'range 함수': for문은 숫자리스트를 자동으로 만들어 주는 range 함수와 함께 사용하는 경우가 많다.
a = range(10)
print(a)  # range(0, 10)
# range(10)은 0부터 10 미만의 숫자를 포함하는 range 객체를 만들어 준다.

# 시작, 끝 숫자를 지정하려면 range(시작 숫자, 끝 숫자) 형태를 사용하는데, 이때 끝 숫자는 포함되지 않는다.
a = range(1, 11)
print(a)  # range(1, 11)


# range 함수 예시:
# for와 range 함수를 사용하면 1부터 10까지 더하는 것을 다음과 같이 구현.
add = 0
for i in range(1, 11):
    add = add + i
    print(add)  # 55
# range(1, 11)은 숫자 1부터 10까지(1이상 11미만)의 숫자를 데이터로 갖는 객체.
# 따라서 위 i변수에 리스트의 숫자가 1부터 10까지 하나씩 차례로 대입되면서, add = add + i 문장을 반복수행하고 add는 최종적으로 55가 된다.
...
# 3번 학생 축하합니다. 합격입니다.
# 5번 학생 축하합니다. 합격입니다.
# len 함수는 리스트의 요소 갯수를 돌려주는 함수. 따라서 len(marks)는 5, range(len(marks))는 range(5).
# number 변수에 0~4까지의 숫자가 대입되고, marks[number]는 차례대로 90, 25, 67, 45, 80 값을 갖게 된다.


# for와 range 함수를 이용한 구구단: 소스 코드 단 4줄만으로 구구단을 출력할 수 있다.
...
# for문 두 번 사용. ①번 for문에서 2부터 9까지의 숫자(range(2, 10))가 차례로 i에 대입. i가 처음 2일 때, ②번 for문을 만나게 된다.
# ②번 for문에서 1부터 9까지의 숫자(range(1, 10))가 j에 대입, 그다음 문장 print(i*j)를 수행.
# 따라서 i가 2일 때 2*1, 2*2, 2*3, … 2*9까지 차례로 수행되며 그 값을 출력. i가 3일 때 역시 2일 때와 마찬가지로 수행될 것이고 i가 9일 때까지 계속 반복.

# [매개변수 end를 넣어 준 이유는 무엇일까?]
# 예제에서 print(i*j, end=" ")와 같이 매개변수 end를 넣어 준 이유는, 해당 결괏값을 출력시 다음줄로 넘기지 않고 그 줄에 계속해서 출력하기 위해서.
# 그다음에 이어지는 print(' ')는 2단, 3단 등을 구분하기 위해 두 번째 for문이 끝나면 결괏값을 다음 줄부터 출력하게 해주는 문장.


# 10-6. 리스트 내포
# 리스트에 for문을 포함하는 '리스트 내포(List comprehension)'를 사용하면 더 편리하고 직관적인 프로그램을 만들 수 있다.
...
# 위는 a리스트의 각 항목에 3을 곱한 결과를 result 리스트에 담는 예제.
...
# 만약 [1,2,3,4] 중 짝수만 3을 곱해 담을땐 리스트 내포에 "if 조건" 사용.
a = [1, 2, 3, 4]
result = [num * 3 for num in a if num % 2 == 0]
print(result)  # [6, 12]

# 리스트 내포의 일반 문법: "if 조건" 부분은 생략가능.
# [표현식 for 항목 in 반복가능객체 if 조건문]
# 조금 복잡하지만 for문을 2개 이상 사용도 가능. for문을 여러 개 사용시 문법:
...
# 만약 구구단의 결과를 리스트에 담을때 리스트 내포를 사용해 다음과 같이 구현할 수도 있다.

result = [x*y for x in range(2, 10)
          for y in range(1, 10)]
print(result)
# [2, 4, 6, 8, 10, 12, 14, 16, 18, 3, 6, 9, 12, 15, 18, 21, 24, 27, 4, 8, 12, 16, 20, 24, 28, 32, 36, 5, 10, 15, 20,
# 25, 30, 35, 40, 45, 6, 12, 18, 24, 30, 36, 42, 48, 54, 7, 14, 21, 28, 35, 42, 49, 56, 63, 8, 16, 24, 32, 40, 48, 56,
# 64, 72, 9, 18, 27, 36, 45, 54, 63, 72, 81]


# [예제 1~9]
# 예제1)
# 학생들에게 최종성적을 알려주는 '학점 계산기'를 만들려고 합니다.
...
# D: 60점 이상 70점 미만
# F: 60점 미만
# print_grade 함수는 파라미터로 중간고사 점수 midterm_score와 기말고사 점수 final_score를 받고, 최종성적 출력.
...
print_grade(50, 45)
# 결과: B, F, D, A
# 해설: B를 받기 위해서는 총 점수가 '80점 이상이면서 90점 미만' 이어야.
# 위에 작성된 조건을 보면 80 <= total < 90이 아니라 그냥 total >= 80입니다. 왜 그런 걸까요?
# elif 문으로 넘어 왔다는 것은 앞선 if 문의 조건 부분을 통과하지 않았다는 뜻.
# 그러니까 점수가 90점 미만일 수밖에 없다는 거죠.


# 예제2)
# while 문과 if 문을 활용하여, 100 이하의 자연수 중 8의 배수이지만 12의 배수는 아닌 것을 모두 출력.
# 예를 들어서 16은 8의 배수이지만 12의 배수가 아니니까 조건에 부합합니다. 하지만 48은 8의 배수이면서 12의 배수이기도 해 조건에 부합하지 않습니다.
...
# 참고로 이 문제는 '중첩 while문' 이라는 개념을 사용해야 하는데요. 중첩 while 문은 while 문의 수행부분 안에 또 다른 while 문을 넣는 것.
