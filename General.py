...
# 출처: https://www.codingfactory.net/11753
...
# 추상화(Abstraction): 복잡한 내용을 숨기고 주요 기능에만 신경쓰도록.
...
# 4) White space(화이트 스페이스): 들여쓰기는 무조건 '4스페이스' 사용.
def say_hello():
    print("Hello, Harry!")  # 들여쓰기 4번


# 5) 함수 정의: 함수 정의 '위,아래로 빈 줄이 2개씩.' 파일의 첫 줄이 함수면 괜찮다.
def a():  # 위,아래 빈 줄 2줄씩.
    print('a')


# 6) 괄호 안: 괄호 바로 안은 띄어쓰기 하지 말 것.
# spam( ham[ 1 ], { eggs: 2 } )  # 띄어쓰기의 안좋은 예.


# 7) 함수 괄호: 함수 정의/호출시, 함수 이름과 골호 사이 띄어쓰기 하지 말 것.
# def spam  <함수명 <-> 괄호 사이 띄어쓰기 x> (a):
def spam(a):
    print(x + 2)


# 8) 쉼표: 쉼표 앞은 띄어쓰기 하지 말 것.
print(a, a)  # good     print(a , b) >  # bad

# 9) 지정연산자: 지정연산자 앞뒤로 띄어쓰기 한 번.
x = 1  # good

# 10) 연산자: 기본적으로 연산자 앞뒤로 띄어쓰기 한 번.
x = x + 1
x += 1

# 11) + 하지만 연산의 '우선순위' 강조를 위해 연산자 앞뒤로 띄어쓰지 않고 붙이길 권장.
x = x*2 - 1
x = x*x + x*x
c = (x+x) * (x-x)

# 12) 코멘트: 일반 코드와 같은 줄에 코멘트를 쓸 경우, 코멘트 앞 띄어쓰기 최소 두 개.
x = x + 1  # 코멘트

# +) 코딩 스타일: 이해하기 쉽고 가독성이 좋은 코드가 좋은 스타일을 가진 좋은 코드.
...
# 반올림 표시: 원하는 파라미터 뒤에 ':.(n)f'  # 소숫점 이하 몇자리까지 표시지정  # 1 나누기 3은 0.33입니다.
...
# 내장함수 사용시 문자열 변수이름 뒤 ‘.’를 붙인 다음에 함수이름을 써주면 된다.
...
# 6> 이스케이프 문자(escape character): 원래 가지고 있던 문자열의 출력하는 기능을 벗어나 다른 특정한 기능을 하도록 하는 문자.
#                                    \(백슬래시) 기호가 붙은 특수한 문자.
# 1) \n : 다음 줄로 이동(개행)
# 2) \r :해당 줄의 처음으로 이동
# 3) \t : '8칸' 공백
...
# 9> 문자열 인덱싱(indexing): 문자열 가리키기. 'a[번호]'는 문자열 안의 특정한 값을 뽑아내는 역할.(리스트나 튜플에서도 사용가능)
...
# a[-1] == 'n'
# a[-0] == 'L' == a[0]
# 문자열을 뒤에서부터 읽기위해 마이너스(-) 기호를 붙임. 즉, a[-1]은 뒤에서 첫 번째가 되는 문자.
...
# 10> 문자열 슬라이싱(slicing): 문자열에서 특정한 단어를 뽑아내는 방법.(리스트나 튜플에서도 사용가능)
# 1) 단순접근:
a = "Life is too short, You need Python"
b = a[0] + a[1] + a[2] + a[3]
print(b)  # 'Life'
...
# 슬라이싱을 사용하면 "Pithon" 문자열을 'P' 부분과 'thon' 부분으로 나눌 수 있고 그 사이에 'y' 문자를 추가해, 'Python' 이라는 새로운 문자열을 만듦.


# 11> 불린(Boolean): 따옴표 없이, 첫자는 대문자!
...
# 13> 리스트(List): 리스트는 자료들을 목록화시킨 데이터의 묶음. 여러 변수를 한 번에 묶어 하나의 변수로 사용할 수 있다.
...
# 1) 리스트의 인덱싱
# 리스트 역시 문자열처럼 인덱싱을 적용할 수 있다. 먼저 a 변수에 [1, 2, 3] 값을 설정.
a = [1, 2, 3]
print(a)  # [1, 2, 3]
...
# 리스트의 첫 번째 요소인 a[0]와 세 번째 요소인 a[2]의 값을 더함.
print(a[0] + a[2])  # 1 + 3 = '4'

# a[-1]은 문자열에서와 마찬가지로 리스트 a의 마지막 요솟값.
print(a[-1])  # 3

# 리스트 a를 숫자 1, 2, 3과 또 다른 리스트인 ['a', 'b', 'c']를 포함하도록 만들면,
a = [1, 2, 3, ['a', 'b', 'c']]
print(a[0])  # 1
print(a[-1])  # ['a', 'b', 'c']
print(a[3])  # ['a', 'b', 'c']
# a[-1]은 마지막 요솟값 ['a', 'b', 'c']를 나타냄. a[3]은 리스트 a의 네 번째 요소를 나타내서, 마지막 요소를 나타내는 a[-1]과 동일한 결괏값.
...
# 이떄 'Life' 문자열만 끄집어내려면,
print(a[2][2][0])  # 'Life'
# 리스트 a의 세 번째 요소 리스트 ['a', 'b', ['Life', 'is']]에서 세 번째 요소인 리스트 ['Life', 'is']의 첫 번째 요소를 나타냄.
# 리스트를 삼중으로 중첩해서 쓰면 혼란스러워 자주 사용하지 않는다.


# 2) 리스트의 슬라이싱
# 문자열과 마찬가지로 리스트에서도 슬라이싱 기법을 적용할 수 있다. 슬라이싱은 ‘나눈다’는 뜻으로, 문자열에서 했던 것과 사용법이 완전히 동일하다.
a = [1, 2, 3, 4, 5]
print(a[0:2])  # [1, 2]
# 앞의 예를 문자열에서 슬라이싱했던 것과 비교해 보자.  # 2가지가 완전히 동일하게 사용.
a = "12345"
print(a[0:2])  # '12'
...
print(c)  # [3, 4, 5]  # c라는 변수는 리스트 a의 세 번째 요소부터 끝까지 나타내는 리스트.


# [중첩된 리스트에서 슬라이싱]
# 리스트가 포함된 중첩 리스트 역시 슬라이싱 방법은 똑같이 적용.
a = [1, 2, 3, ['a', 'b', 'c'], 4, 5]
...
# [1, 2, 3] 리스트가 세 번 반복돼 새로운 리스트를 만들어냄. 문자열에서 "abc" * 3 = "abcabcabc" 가 되는 것과 같다.


# 4) 리스트 길이구하기: len 함수 사용.
a = [1, 2, 3]
print(len(a))  # 3
# len 함수는 문자열, 리스트 외 튜플과 딕셔너리에도 사용할 수 있다.


# 5) [초보자가 범하기 쉬운 리스트 연산오류]
# 예제) 다음 소스 코드를 입력했을 때 결괏값은?
# a = [1, 2, 3]
# print(a[2] + "hi")
# a[2]의 값인 3과 문자열 hi가 더해져서 3hi가 출력될 것이라고 생각할 수 있다. 하지만, 형 오류(TypeError)가 발생한다.
# Traceback (most recent call last):  # TypeError: unsupported operand type(s) for +: 'int' and 'str'
# a[2]에 저장된 3은 정수, "hi"는 문자열이다. 정수와 문자열은 더할 수 없기 때문에 형 오류가 발생한다.

# 숫자와 문자열을 더해 '3hi' 처럼 만들고 싶다면 숫자 3을 문자 '3'으로 바꿔야 한다.
str(a[2]) + "hi"
