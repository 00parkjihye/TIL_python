...
# 7) 함수 괄호: 함수 정의/호출시, 함수이름과 괄호사이 띄어쓰기 하지 말 것.
...
# 13> 리스트(List): 리스트는 자료들을 목록화시킨 데이터의 묶음. 여러 변수를 한번에 묶어 하나의 변수로 사용할 수 있다.
...
# 7) 리스트 요소 끄집어내기(pop): '.pop()' 리스트 맨 마지막 요소를 돌려주면서, 그 요소를 삭제.
a = [1, 2, 3]
print(a.pop())  # 3
print(a)  # [1, 2]  # 리스트에서 3을 끄집어내고, 최종적으로 [1, 2]만 남음.

# '.pop(삭제할 자릿수a)' 리스트의 a번째 요소를 끄집어내고, 그 요소를 삭제.
a = [1, 2, 3]
print(a.pop(1))  # 2
print(a)  # [1, 3]


# 8) 리스트에 포함된 요소 x의 개수 세기(count): '.count(x)'
# count(x)는 리스트 안에 x가 몇 개인지 그 갯수를 돌려주는 함수.
a = [1, 2, 3, 1]
print(a.count(1))  # 2


# 9) 리스트 확장(extend): '.extend(x)'
# extend(x)에서 x에는 리스트만 올 수 있으며, 원래의 a 리스트에 x 리스트를 더하게 됨.
a = [1, 2, 3]
a.extend([4, 5])
print(a)  # [1, 2, 3, 4, 5]
...
# 10) [초보자가 범하기 쉬운 리스트 연산오류]
# 예제) 다음 소스 코드를 입력했을 때 결괏값?

a = [1, 2, 3]
# print(a[2] + "hi")
# a[2]의 값인 3과 문자열 hi가 더해져서 3hi가 출력될 것이라고 생각할 수 있다. 하지만, 형 오류(TypeError)가 발생.
# Traceback (most recent call last):  # TypeError: unsupported operand type(s) for +: 'int' and 'str'
# a[2]에 저장된 3은 정수, "hi"는 문자열이다. 정수와 문자열은 더할 수 없기 때문에 형 오류가 발생함.
