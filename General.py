...
# 7> 'break 문': 만약 while문의 조건부분과 상관없이 '반복문에서 나오고 싶으면', break문 사용.
i = 100
while True:
...
# 답: 무엇이 참이라기 보다 그냥 참을 의미하는 'Boolean값 그 자체'를 의미.
# 그래서 while문 다음에 나오는 while표현식 부분이 True로 평가되는 것들을 사용하실 수 있어요.
# False, None, 0, '', (), [], {}
# 이런 것들이 False로 평가되는데, 이외의 값들, 예를 들어 while 1과 같은 것들은 '1'이 'True'로 평가되어 같은의미로 사용가능.


# 8> 'continue 문': 현재 진행되고 있는 수행부분을 중단하고, '바로 조건부분을 확인'하고 싶으면 continue문 사용.
i = 0
while i < 15:
    i = i + 1

    if i % 2 == 1:  # i가 홀수면 print(i) 하지않고, 바로 조건부분으로 돌아감.
        continue
