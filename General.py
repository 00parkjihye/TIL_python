...
# 2> 'if...else~' 문': if(만약), else(그렇지 않으면). 상황별로 다른 동작을 위해서 사용.
...
# 'elif 문': 수행부분이 여러개 있지만, 단 하나의 수행부분만이 실행 될 수 있음.
...
# 수업에는 50점 만점의 중간고사와 50점 만점의 기말고사가 있는데요. 두 시험의 점수를 합해서 최종 성적을 내는 방식입니다.
# 규칙은 다음과 같습니다.
# A: 90점 이상
# B: 80점 이상 90점 미만
# C: 70점 이상 80점 미만
